...
const Productdetails = React.memo(({ item }) => {
  ...
  const count = cartItem.flatMap((item) => item.count).join();
  console.log(
    "🚀 ~ file: Productdetails.js ~ line 19 ~ Productdetails ~ count",
    count
  );
  const id = cartItem.map((item) => item.id).filter((id) => id === item.id);
  console.log(item.id === id);
  console.log(
    "🚀 ~ file: Productdetails.js ~ line 20 ~ Productdetails ~ id",
    id
  );
  // index photo
