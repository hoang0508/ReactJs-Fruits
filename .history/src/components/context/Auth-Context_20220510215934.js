...
const ImageProvider = ({ images, children, totalValues, ...props }) => {
  ...
  console.log(
    "🚀 ~ file: Auth-Context.js ~ line 29 ~ ImageProvider ~ cartItem",
    cartItem
  );
  // Count
  ...
  console.log(
    "🚀 ~ file: Auth-Context.js ~ line 42 ~ ImageProvider ~ total",
    total
  );
  useEffect(() => {
  ...
  // Total reducer
  const totalReducer = cartItem.reducer((prev, current) => prev + current, 0);
  console.log(
    "🚀 ~ file: Auth-Context.js ~ line 57 ~ ImageProvider ~ totalReducer",
    totalReducer
  );
