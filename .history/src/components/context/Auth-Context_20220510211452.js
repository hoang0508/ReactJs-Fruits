...
const ImageProvider = ({ images, children, item, ...props }) => {
  console.log(
    "🚀 ~ file: Auth-Context.js ~ line 8 ~ ImageProvider ~ item",
    item
  );
  ...
  // Total
  const [total, setTotal] = useState(props.priceNew);
  ...
  const addToCart = (newItem) => {
    setCartItem((prevItem) => {
      ...
      if (isExisted) {
        ...
        console.log(
          "🚀 ~ file: Auth-Context.js ~ line 57 ~ setCartItem ~ totalValue",
          totalValue
        );
        setTotal(totalValue);
        return [...prevItem];
